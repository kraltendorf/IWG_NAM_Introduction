...
# prepare backbone
backbone <- read.csv(paste(in_path, "backbone.csv", sep = "/"))
...
for (i in 1:length(traits)) {
  file_names <- list.files(paste(in_path, "/", traits[i], sep = ""), pattern = "emmeans_genet", full.names = T)
  ...
  for (j in 1:length(files)) {
    ...
    colnames(file)[2] <- paste(str_split(traits[i], "_")[[1]][1], "_", env_names[j], sep = "")
    files_ed[[j]] <- file
  }
...
# join together
all_phenotypes <- left_join(output[[1]], output[[2]], by = "longID")
...
for (j in 1:length(famID)) {
  loc <- read.table(paste(dir, "Additional Filtering and JoinMap File Prep/output/", famID[j], "_chr1.txt", sep = ""), skip = 7, fill = TRUE)
