...
## are located
INDIR=/scratch.global/kaltendo/gatk_temp/NAM_GATK/BAM_Processing/NAM_GATK/Processed

# The directory in which the .g.vcf files should be placed
## Note: a scratch directory is recommended
OUTDIR=/scratch.global/kaltendo/gatk_temp/NAM_GATK/HaplotypeCaller
...
## 0.01
## -DF disable mappingquality available read filter as it eliminates ALL reads.


GATK_SETTINGS='-DF NotDuplicateReadFilter -ERC GVCF --heterozygosity 0.01 -DF MappingQualityAvailableReadFilter -L Chr01 -L Chr02 -L Chr03 -L Chr04 -L Chr05 -L Chr06 -L Chr07 -L Chr08 -L Chr09 -L Chr10 -L Chr11 -L Chr12 -L Chr13 -L Chr14 -L Chr15 -L Chr16 -L Chr17 -L Chr18 -L Chr19 -L Chr20 -L Chr21'

# File path up to and including the GBarleyS directory 
## (i.e. path/to/GBarleyS)
VCPWD=/home/janderso/kaltendo/WGN_GATK/GBarleyS
...
# I removed path/to/GATK.jar here
...
GATK_SETTINGS:${GATK_SETTINGS} PROJECT:$PROJECT QUEUE:$QUEUE STAGE:$STAGE )
...
mkdir -p $OUTDIR/$PROJECT/${STAGE}_Variants
...
gatk --java-options "-Xmx64g" HaplotypeCaller -R $REFERENCE -I "'$SAMPLE'" ${GATK_SETTINGS} -O "'$OUTFILE'"; }; \
export -f haplocall; \
