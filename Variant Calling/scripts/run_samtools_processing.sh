...
INDIR=/scratch.global/kaltendo/gatk_temp/NAM_GATK/Read_Mapping/ReadMap_Output/NAM_GATK

# Directory in which the processed .bam files should be placed
OUTDIR=/scratch.global/kaltendo/gatk_temp/NAM_GATK/BAM_Processing
...
QUEUE_SETTINGS='-l walltime=15:00:00,mem=4g,nodes=1:ppn=16'
...
samtools view -hF 4 "'$bam'" | grep -v "'"XS:i"'" | samtools view -Su - | samtools sort -O bam -o $OUTDIR"'/Processed/${bamname}_processed'".bam; \
samtools flagstat $OUTDIR"'/Processed/${bamname}_processed.bam'" > $OUTDIR"'/Stats/${bamname}_processed_stats.out'"; \
samtools index -m 14 $OUTDIR"'/Processed/${bamname}_processed.bam'"; }; \
